...
{
  double val = ( (4*gx*gx*e*e)/(pow(t(x,p) - mv*mv, 2)) )*(4*x[0]*x[0]*mn*mn*gt*ct + x[0]*x[0]*gt*cc*(t(x,p) - 2*mn*mn) + mn*mn*gc*ct*(t(x,p) - 2*x[0]*x[0] ) )
             + ( (2*gx*gx*e*e)/(pow(t(x,p) - mv*mv, 2)) )*gc*cc*( pow( s(x,p) - x[0]*x[0] - mn*mn, 2) + pow(u(x,p) - x[0]*x[0] - mn*mn, 2) )
             + ( (16*gx*gx*e*e)/(pow(t(x,p) - mv*mv, 2)) )*ga*ca*(6*gv*cv*pow(s(x,p) - x[0]*x[0] - mn*mn, 2) + (( 2*ga*ca*x[0]*x[0]*mn*mn*t(x,p) )/(mv*mv)) + (( ga*ca*x[0]*x[0]*mn*mn*t(x,p)*t(x,p) )/(mv*mv*mv*mv))  );
    return val;
}
TF1 *GR_1;
...
void Matrixelementdependonmx()
{
GR_1 = new TF1("", GR, 0, 1, 1);
NGR_1 = new TF1("", NGR, 0, 1, 1);
TCanvas *c1 = new TCanvas("c1","Matrix element depend on mass of DM", 900, 700);
gPad->SetTicks(1,1);
c1->SetLogy();
...
GR_1->SetTitle("; m_{#chi} (GeV);  #LT|M|^{2}#GT");
GR_1->SetLineColor(2);
GR_1->SetLineWidth(3);
GR_1->SetMaximum(1E10);
GR_1->SetMinimum(6E-2);

NGR_1->GetXaxis()->CenterTitle();
NGR_1->GetYaxis()->CenterTitle();
NGR_1->SetTitle("; m_{#chi} (GeV); #LT|M|^{2}#GT");
...
TLegend *legend = new TLegend( 0.65, 0.7, 0.85, 0.89);
