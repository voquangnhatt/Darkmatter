...
//    return ( (1./32.)*pow(sin(2*a), 2)*yx*yx*ghnn*ghnn*pow(h*h - mh1*mh1, 2)*(5*mx*mx -2*t)*(5*mn*mn -2*t) ) / ( pow(t - h*h, 2)*pow(t - mh1*mh1, 2) )  ;
    ...
    {
      double val = ( (1./32.)*pow(sin(2*al), 2)*yx*yx*ghnn*ghnn*pow(x[0]*x[0] - mh1*mh1, 2)*(5*mx*mx -2*t)*(5*mn*mn -2*t) ) / ( pow(t - x[0]*x[0], 2)*pow(t - mh1*mh1, 2) );
      	return val;
    }
    TF1 *EQ_1;
...
void MdependonDarkHiggs()
{
  EQ_1 = new TF1("", EQ, 0.0, 200.0, 1);
  EP_1 = new TF1("", EP, 0.0, 200.0, 1);
  ET_1 = new TF1("", ET, 0.0, 200.0, 1);
  ...
  EQ_1->SetTitle("; m_{h_{2}} (GeV); Matrix element (#LT|M|^{2}#GT)");
  EQ_1->SetLineColor(2);
  EQ_1->SetLineWidth(3);
  EQ_1->SetMaximum(1E4);
  ...
  EP_1->SetTitle("; m_{h_{2}} (GeV); Matrix element (#LT|M|^{2}#GT)");
  EP_1->SetLineColor(4);
  EP_1->SetLineWidth(3);

  ET_1->GetXaxis()->CenterTitle();
  ET_1->GetYaxis()->CenterTitle();
  ET_1->SetTitle("; m_{h_{2}} (GeV); Matrix element (#LT|M|^{2}#GT)");
  ET_1->SetLineColor(1);
  ET_1->SetLineWidth(3);
...
//  EQ_1->Draw("same");
  ...
  TLegend *legend = new TLegend( 0.65, 0.5, 0.85, 0.75);
  legend->SetHeader("#alpha = 40^{0}","c");
